...
int counterOutput[] = {0, 0, 0, 0, 0, 0, 0};
...
void Encrypt()
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        {
            swap(combined, 8);
            copy(combined, counter, 8);
        }
    }

    finalPerm(combined);

    copy(combined, counterOutput, 8);
}
...
void encryptP(unsigned char *pix, int size)
{
    for (int i = 0; i < size; i++)
    {
        ...
        Encrypt();
        xorArrays(plaintext, counterOutput, 8, ciphertext);
        pix[i] = binaryToChar(ciphertext);
        incrementBinaryArray(counter, 8);
    }
}
...
void decryptP(unsigned char *pix, int size)
{
    for (int i = 0; i < size; i++)
    {
        ...
        Encrypt();
        xorArrays(ciphertext, counterOutput, 8, plaintext);
